...
var check_exist_table = require(CONF.oft_modules + 'check_exist_table.js');

// 2 суток в секундах, используем чтобы не учитывать всех слушателей,
// которые больше этого времени слушают
var HOW_LONG_LISTEN = 172800;
...
function stations_get_exist_tables (CONTEXT, cb_main) {
  check_exist_table.stations(CONTEXT.get('station'), function(err, res) {
    if (!err) { CONTEXT.set('stations_exist_tables', res); }
      cb_main(err || null, 'check_exist_table.stations');
  });
}
...
function get_listeners_in_current_day (CONTEXT, cb_main) {
  var get_data = function (date, add_params, cb) {
    ...
    var query = "SELECT DATE_FORMAT(date, '%Y-%m-%d') as date, SUM(duration) as totalSeconds FROM `stations_"+CONTEXT.get('station')+"`.`"+date+"` WHERE mount='"+CONTEXT.get('stream')+"' AND duration<="+HOW_LONG_LISTEN;
    // console.log(query)
...
function get_listeners_who_start_in_past_day (CONTEXT, cb_main) {
  var get_data = function (date, add_params, cb) {
    var CONTEXT = add_params[0];
    var query = "SELECT date, duration FROM `stations_"+CONTEXT.get('station')+"`.`"+date+"` WHERE mount='"+CONTEXT.get('stream')+"' AND UNIX_TIMESTAMP(date)-duration<UNIX_TIMESTAMP('"+date+"') AND duration<="+HOW_LONG_LISTEN;
    // console.log(query);
    db.read(query, function(err, res) {
      cb(err || null, [date, res || null]);
    });
