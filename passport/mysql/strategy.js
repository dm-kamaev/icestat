...
module.exports = function(passport) {
    ...
    passport.use(
        ...
        function(req, username, password, done) {
            var isCaptchaValid = (req.body.captcha == req.session.captcha);
            if (isCaptchaValid) {
                // find a user
                ...
                connection.query("SELECT * FROM users WHERE username = ?",[username], function(err, rows) {
                    ...
                    if (rows.length) {
                        ...
                        connection.query(insertQuery,[newUserMysql.username, newUserMysql.password, newUserMysql.email],function(err, rows) {
                            ...
                            if (newUserMysql.id == 1) { // first user is always admin user
                                connection.query("UPDATE users SET admin=1 WHERE id=1", function(err, rows) {
                ...
                return done(null, false, req.flash('signupMessage', 'Captcha is not valid.'));
            }
        })
    );
