...
router.post('/', function(req, res, next) {
    ...
    var sort = (req.query.jtSorting) ?  " ORDER BY permissions." + req.query.jtSorting : " ORDER BY permissions.id DESC ";
    ...
    async.waterfall([
        ...
        function(rows, cb) {
            con.query('SELECT COUNT(*) as total from `permissions`', function(err, res){ cb(err, res); });
        },
        function(rows, cb) {
            var val = req.body,
            sql = "SELECT permissions.id as perm_id, permissions.selected as selected, user_id, mount_id, mounts.name as mount_name, users.username as user_name FROM permissions " +
                ...
                "JOIN mounts ON mounts.id = permissions.mount_id " + sort + " LIMIT " + startIndex + ", " + pageSize;
