...
router.post('/', function(req, res, next) {
    ...
    async.map(queries, getAQHListenersFromDB.bind({pool: pool}), function(err, results) {
        var data = [];
...
function getAQHListenersFromDB(q, callback) {
    var pool = this.pool;
...
function formatQuery(timeRange, timeStep, db_name, table_name, mount) {
    var sp = 'CALL GetAQHListeners(@RANGE_START, @RANGE_END, @RANGE_STEP, @DB_NAME, @TABLE_NAME, @MOUNT)';
