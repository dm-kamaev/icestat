...
function parsePlaylistLog(line) {
    ...
    var author = '', song_name = '';
    if (matches[3]) {
      var authorSong_name = matches[3].replace(/\s+/, ' ').replace(/^\s+-/, '').split('-');
      author    = (authorSong_name[0]) ? authorSong_name[0].trim() : '';
      song_name = (authorSong_name[1]) ? authorSong_name[1].trim() : '';
    }
    return {
