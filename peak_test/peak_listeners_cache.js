...
var my_date=require('/data/node/radiostat/date.js');
var my_show_tables=require('/data/node/radiostat/show_tables.js');
var my_sql=require('/data/node/radiostat/sql.js');
...
function get_data_from_table(db, mount, day, Highcharts_res, Table_res, callback) {
  ...
  var cache_key=db+'_'+day+'_'+mount;
  if (CACHE[cache_key]) {
  ...
  function process(mysql_res) {
    var ConCurrent=[];        for(var i=0; i<max_step; i++) { ConCurrent[i]=0; }
